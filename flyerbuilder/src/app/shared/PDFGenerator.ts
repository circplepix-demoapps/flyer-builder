...
export class PDFGenerator {
   ...
   processImageData(sources: any[], callback: any){
      //TODO: dapat ma automatic tni ky kinanglan pa ta mo dugang dri ug doorhanger ug postcard dynamically
      ...
      this.loadImages(sources, function(images: any[]) {
         ...
         for(var i in images) {
            context.drawImage(images[i], parseInt(images[i].left) * hightQualityMuliplier, parseInt(images[i].top) * hightQualityMuliplier, 
                                       parseInt(images[i].width) * hightQualityMuliplier, parseInt(images[i].height) * hightQualityMuliplier);
         }
   ...
   generatePDF(sources: any, callback: any) {
      ...
      //TODO: kinanglan ni checkon ky murag temporari tni nga fix sa coodinate sa text
