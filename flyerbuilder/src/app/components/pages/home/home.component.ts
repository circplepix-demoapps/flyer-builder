...
export class HomeComponent implements OnInit, AfterViewInit, OnDestroy {
   ...
   constructor(private dialogsService: DialogService, private flyerLocalStorage: FlyerLocalStorageService) {
   }
   ...
   ngAfterViewInit() {
      ...
      * Notes: feel free to refactor codes
      /*********************************************/
      ...
      var draggableOption = {
         ...
         start: function () {
            if ($(this).hasClass(configConstant.FLYER_ELEMENT_BLANK)) {
               $(this).css({ 'z-index': 1 });
            } else {
      ...
      var droppableOption = {
         ...
         drop: function (ev: any, ui: any) {
            ...
            drp_element = handleWhichElement($(active_element), element_id, x, y, width, height);
      ...
      * Notes: feel free to refactor codes
      ...
      function handleWhichElement(active_el: any, id: any, x: any, y: any, width: any, height: any) {
         if(!active_el) {
            return;
         }

         var el = {
            'id': id, 'x': x, 'y': y, 'width': width, 'height': height, 'src': '', 'value': ''
         };

         if (active_el.hasClass(configConstant.FLYER_ELEMENT_STILL)) {
            el['type'] = ElementType.image;
            el['src'] = configConstant.STILL_DEFAULT_IMAGE;
         }
         else if (active_el.hasClass(configConstant.FLYER_ELEMENT_TEXT)) {
            el['type'] = ElementType.text;
            el['value'] = 'THIS IS A SAMPLE TEXT';
         }
         else if (active_el.hasClass(configConstant.FLYER_ELEMENT_STATIC)) {
            el['type'] = ElementType.static;
            el['src'] = configConstant.STATIC_DEFAULT_IMAGE;
         }
         else if (active_el.hasClass(configConstant.FLYER_ELEMENT_AGENT)) {
            el['type'] = ElementType.agent;
            el['src'] = configConstant.AGENT_DEFAULT_IMAGE;
         }
         else if (active_el.hasClass(configConstant.FLYER_ELEMENT_LOGO)) {
            el['type'] = ElementType.logo;
            el['src'] = configConstant.LOGO_DEFAULT_IMAGE;
         }
         else if (active_el.hasClass(configConstant.FLYER_ELEMENT_REALTOR)) {
            el['type'] = ElementType.realtor;
            el['src'] =  configConstant.REALTOR_DEFAULT_IMAGE;
         }
         else if (active_el.hasClass(configConstant.FLYER_ELEMENT_EHO)) {
            el['type'] = ElementType.eho;
            el['src'] = configConstant.EHO_DEFAULT_IMAGE;
         }
         else if (active_el.hasClass(configConstant.FLYER_ELEMENT_QR)) {
            el['type'] = ElementType.qr;
            el['src'] = configConstant.QR_DEFAULT_IMAGE;
         }
         else if (active_el.hasClass(configConstant.FLYER_ELEMENT_BULLET)) {
            el['type'] = ElementType.bullet;
            el['src'] = configConstant.BULLET_DEFAULT_IMAGE;
         }
         else if (active_el.hasClass(configConstant.FLYER_ELEMENT_TEXTAREA)) {
            el['type'] = ElementType.textarea;
            el['value'] = TestData.LOREM_IPSUM
         }
      ...
      function handleChangeDimension() {
         ...
         if ($('#input-element-width').val().length <= 0) {
            return;
         }

         if ($('#input-element-height').val().length <= 0) {
            return;
         }
