(function ($) {
    $.widget("custom.ruler", {
        ...
        _create: function () {
            ...
            /* Create container */
            var $container = $(document.createElement('div')).addClass('rsf-ruler');
    ...
    _onWindowResize:function() {
        ...
        _fixArrowsPosition: function (mouseX, mouseY) {
            var arrowX = mouseX - this.element.offset().left - this._$corner.width() - Math.round(this._$topArrow.outerWidth() / 2) + parseInt($('.rsf-ruler').parents().eq(1).css('padding-left'),10);
            var arrowY = mouseY - this.element.offset().top - this._$corner.height() - Math.round(this._$leftArrow.outerHeight() / 2) + parseInt($('.rsf-ruler').parents().eq(1).css('padding-top'),10);
